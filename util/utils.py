...
def learning_curve_with_cross_validation(
    estimator, X, y, train_sizes, cv=5, scoring=None, dataset_name=None
):
    ...
    if dataset_name:
        plt.savefig(
            f"figures/{clf_name}_{dataset_name}_learning_curve.pdf", bbox_inches="tight"
        )
    else:
...
def perform_grid_search(
    ...
    if show_plot:
        ...
        if dataset_name:
            plt.savefig(
                f'figures/{str(model).split("(")[0]}_{dataset_name}_grid_search_{target_feature}.pdf',
                bbox_inches="tight",
            )
        plt.savefig(
            f'figures/{str(model).split("(")[0]}_grid_search_{target_feature}.pdf',
            bbox_inches="tight",
        )
        plt.show()
...
def plot_pca_cumulative_explained_variance_ratio(
    X, pca, dataset_name="Digits", ratio_threshold=0.9
):
    # Fit PCA on the scaled training data
    pca.fit(X)
    ...
    plt.axhline(
        ...
        color="r",
        linestyle="--",
        label=f"{ratio_threshold} threshold",
    )
    plt.xlabel("Number of Components")
    ...
    plt.xticks(np.arange(0, len(eigenvalues) + 1, 5))
    plt.savefig(
        f"figures/DR/{dataset_name}_pca_cumulative_explained_variance_ratio.pdf",
        bbox_inches="tight",
    )
    plt.show()
...
def plot_pca_eigen_and_variance(X, pca, dataset_name="Digits", ratio_threshold=0.8):
    ...
    plt.tight_layout()
    plt.savefig(
        f"figures/DR/{dataset_name}_pca_eigen_and_variance.pdf", bbox_inches="tight"
    )
    plt.show()
...
def plot_ica_mean_kurtosis(
    X, n_min, n_max, dataset_name="Digits", x_ticks_interval=5, x_line=16
):
...
def plot_ordered_ica_kurtosis(X, ica, dataset_name="Digits"):
    ...
    plt.savefig(
        f"figures/DR/{dataset_name}_ica_ordered_kurtosis.pdf", bbox_inches="tight"
    )
...
def plot_k_medoid_multi_seed(
    ...
    ax1.set_ylabel(
        "Silhouette Score", color=color, fontdict={"size": 25, "weight": "bold"}
    )
    ax1.plot(range(k_min, k_max), kmedoids_train_sil_mean, marker="o", color=color)
    ax1.fill_between(
        range(k_min, k_max),
        kmedoids_train_sil_mean - kmedoids_train_sil_std,
        kmedoids_train_sil_mean + kmedoids_train_sil_std,
        alpha=0.3,
        color=color,
    )
    ...
    ax2.fill_between(
        range(k_min, k_max),
        kmedoids_train_wcss_mean - kmedoids_train_wcss_std,
        kmedoids_train_wcss_mean + kmedoids_train_wcss_std,
        alpha=0.3,
        color=color,
    )
...
def gaussian_mixture_multi_seed(X, k_min, k_max, covariance_type="full"):
    ...
    for i in tqdm(range(k_min, k_max)):
        ...
        for seed in RANDOM_SEEDS:
            gmm = GaussianMixture(
                n_components=i, covariance_type=covariance_type, random_state=seed
            )
            gmm.fit(X)
...
def plot_gmm_multi_seed(
    ...
    ax1.set_xlabel("Number of clusters", fontdict={"size": 20, "weight": "bold"})
    ax1.set_ylabel(
        "Silhouette Score", color=color, fontdict={"size": 20, "weight": "bold"}
    )
    ax1.plot(range(k_min, k_max), gmm_train_sil_mean, marker="o", color=color)
    ax1.fill_between(
        range(k_min, k_max),
        gmm_train_sil_mean - gmm_train_sil_std,
        gmm_train_sil_mean + gmm_train_sil_std,
        alpha=0.3,
        color=color,
    )
    ...
    ax2.fill_between(
        range(k_min, k_max),
        gmm_train_bic_mean - gmm_train_bic_std,
        gmm_train_bic_mean + gmm_train_bic_std,
        alpha=0.3,
        color=color,
    )
    ...
    plt.grid(False)
    plt.savefig(
        f"figures/CLUSTERING/GMM_search_{covariance_type}.pdf", bbox_inches="tight"
    )
    plt.show()
...
def gaussian_random_projection_multi_seed(
    X, n_comp_min, n_comp_max, proj_type="Gaussian"
):
        ...
        for seed in [42]:
            if proj_type == "Gaussian":
                grp = GaussianRandomProjection(
                    n_components=n, random_state=seed, compute_inverse_components=True
                )
            else:
                grp = SparseRandomProjection(
                    n_components=n, random_state=seed, compute_inverse_components=True
                )
            X_projected = grp.fit_transform(X)
            reconstruction_error = mean_squared_error(
                X, grp.inverse_transform(X_projected)
            )
...
def plot_RP_metrics(metrics, k_min, k_max):
    ...
    ax1.fill_between(
        range(k_min, k_max),
        np.array(metrics[0]) - np.array(metrics[1]),
        np.array(metrics[0]) + np.array(metrics[1]),
        color=color,
        alpha=0.3,
    )
    ...
    plt.savefig(
        "figures/CLUSTERING/DIGITS/gaussian_random_projection_metrics.pdf",
        format="pdf",
        bbox_inches="tight",
    )
    plt.show()
...
def plot_bar_clustering_metrics_2(X, labels, dataset_name="TITANIC"):
    ...
    plt.savefig(
        f"figures/CLUSTERING/{dataset_name}/cluster_means.pdf", bbox_inches="tight"
    )
    plt.show()
...
def plot_bar_clustering_metrics(X, labels, dataset_name="TITANIC"):
    ...
    cluster_means = cluster_means.drop(
        columns=["alone", "adult", "Cherbourg", "Queenstown", "Southampton"]
    )
